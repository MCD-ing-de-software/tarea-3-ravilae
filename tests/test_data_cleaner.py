...
class TestDataCleaner(unittest.TestCase):
    ...
    def test_drop_invalid_rows_removes_rows_with_missing_values(self):
        ...
        result = cleaner.drop_invalid_rows(df, ["A", "B"])
        #verifica no tiene valores faltantes
        self.assertEqual(len(result), 2, "El número de filas resultantes no es el esperado")
        #verificar df tiene menos filas que el original
        self.assertFalse(result.isnull().values.any(), "El DataFrame resultante todavía contiene valores nulos")
    ...
    def test_drop_invalid_rows_raises_keyerror_for_unknown_column(self):
        ...
        with self.assertRaises(KeyError):
            cleaner.drop_invalid_rows(df, ["Z"])
    ...
    def test_trim_strings_strips_whitespace_without_changing_other_columns(self):
        ...
        original_num = df["num"].copy()
        result = cleaner.trim_strings(df, ["text"])

        expected_text = pd.Series(["abc", "def", "ghi"], name="text")
        pd.testing.assert_series_equal(result["text"], expected_text)
        pd.testing.assert_series_equal(result["num"], original_num)
    ...
    def test_remove_outliers_iqr_removes_extreme_values(self):
        ...
        result = cleaner.remove_outliers_iqr(df, "val")

        self.assertEqual(len(result), 3, "No se eliminó el outlier esperado")
        self.assertNotIn(100, result["val"].values, "El valor outlier 100 sigue presente")
    ...
    def test_remove_outliers_iqr_raises_keyerror_for_missing_column(self):
        ...
        with self.assertRaises(KeyError):
            cleaner.remove_outliers_iqr(df, "no_existe")
    ...
    def test_remove_outliers_iqr_raises_typeerror_for_non_numeric_column(self):
        ...
        with self.assertRaises(TypeError):
            cleaner.remove_outliers_iqr(df, "text")
